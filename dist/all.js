          ...
          if (config.file != null) {
            ...
            if (
              ...
              for (var i = 0; i < config.file.length; i++) {
                formData.append(
                  isFileFormNameString ? fileFormName + i : fileFormName[i],
                  config.file[i],
                  config.file[i].name
                );
              }
          ...
          setup: function () {
            if (!deregisterNewToast) {
              deregisterNewToast = $rootScope.$on(
                ...
                function (event, toasterId, toastId) {
                  for (
                    var i = 0, len = newToastEventSubscribers.length;
                    i < len;
                    i++
                  ) {
            ...
            if (!deregisterClearToasts) {
              deregisterClearToasts = $rootScope.$on(
                ...
                function (event, toasterId, toastId) {
                  for (
                    var i = 0, len = clearToastsEventSubscribers.length;
                    i < len;
                    i++
                  ) {
        ...
        return {
          ...
          link: function (scope, elm, attrs) {
            scope.$watch("directiveName", function (directiveName) {
              if (
                angular.isUndefined(directiveName) ||
              ...
              try {
                directive = $injector.get(
                  attrs.$normalize(directiveName) + "Directive"
                );
              } catch (e) {
                throw new Error(
                  directiveName +
              ...
              if (directiveDetails.scope !== true && directiveDetails.scope) {
                throw new Error(
                  "Cannot use a directive with an isolated scope. " +
                    "The scope must be either true or falsy (e.g. false/null/undefined). " +
                    "Occurred for directive " +
                    directiveName +
                    "."
                );
              }
              ...
              if (directiveDetails.restrict.indexOf("A") < 0) {
                throw new Error(
                  "Directives must be usable as attributes. " +
              ...
              var template = $compile("<div " + directiveName + "></div>")(
                scope
              );
      ...
      function (
        ...
        return {
          ...
          link: function (scope, elm, attrs) {
            ...
            scope.$on("$destroy", function () {
              toasterEventRegistry.unsubscribeToNewToastEvent(
                scope._onNewToast
              );
              toasterEventRegistry.unsubscribeToClearToastsEvent(
                scope._onClearToasts
              );
            });
            ...
            function addToast(toast, toastId) {
              ...
              if (
                mergedConfig["prevent-duplicates"] === true &&
              ...
              if (typeof toast.showCloseButton === "boolean") {
                ...
                if (
                  typeof closeButtonForType !== "undefined" &&
              ...
              if (toast.showCloseButton) {
                toast.closeHtml = $sce.trustAsHtml(
                  toast.closeHtml || scope.config.closeHtml
                );
              }
            ...
            scope._onNewToast = function (event, toasterId, toastId) {
              ...
              if (
                (isUndefinedOrNull(scope.config.toasterId) &&
                  isUndefinedOrNull(toasterId)) ||
                (!isUndefinedOrNull(scope.config.toasterId) &&
                  !isUndefinedOrNull(toasterId) &&
            ...
            toasterEventRegistry.subscribeToClearToastsEvent(
              scope._onClearToasts
            );
          },
          controller: [
            ...
            function ($scope, $element, $attrs) {
              ...
              $scope.click = function (event, toast, isCloseButton) {
                ...
                var tapToDismiss =
                  typeof toast.tapToDismiss === "boolean"
                    ? toast.tapToDismiss
                    : $scope.config.tap;
                if (
                  tapToDismiss === true ||
                  ...
                  if (toast.clickHandler) {
                    if (angular.isFunction(toast.clickHandler)) {
                      ...
                      angular.isFunction(
                        $scope.$parent.$eval(toast.clickHandler)
                      )
                    ) {
